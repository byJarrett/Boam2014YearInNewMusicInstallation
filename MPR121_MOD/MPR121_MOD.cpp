...
int MPR121_MOD_t::getAddress() {
    return address;
}


void MPR121_MOD_t::setRegister(unsigned char reg, unsigned char value){
...
unsigned char MPR121_MOD_t::getRegister(unsigned char reg){
...
bool MPR121_MOD_t::begin(unsigned char address){
...
void MPR121_MOD_t::applySettings(MPR121_MOD_settings_t *settings){
...
mpr121_error_t MPR121_MOD_t::getError(){
    ...
    if((error & (1<<ADDRESS_UNKNOWN_BIT)) != 0){
        ...
        return OUT_OF_RANGE;
                ...
                void MPR121_MOD_t::clearError(){
                    error = 0;
                }

                bool MPR121_MOD_t::isRunning(){
                    return running;
                }

                bool MPR121_MOD_t::isInited(){
                    return (error & (1<<NOT_INITED_BIT)) == 0;
                }

                void MPR121_MOD_t::updateTouchData(){
                    if(!isInited()) return;
                ...
                unsigned int MPR121_MOD_t::getRawTouchData() {
                    return touchData;
                }

                bool MPR121_MOD_t::getTouchData(unsigned char electrode){
                    if(electrode>12 || !isInited()) return false; // avoid out of bounds behaviour

                        return((touchData>>electrode)&1);
                    }

                    unsigned char MPR121_MOD_t::getNumTouches(){
                    ...
                    bool MPR121_MOD_t::getLastTouchData(unsigned char electrode){
                        if(electrode>12 || !isInited()) return false; // avoid out of bounds behaviour

                            return((lastTouchData>>electrode)&1);
                        }

                        bool MPR121_MOD_t::updateFilteredData(){
                        ...
                        int MPR121_MOD_t::getFilteredData(unsigned char electrode){
                            ...
                            int MPR121_MOD_t::getBaselineData(unsigned char electrode){
                                if(electrode>12 || !isInited()) return(0xFFFF); // avoid out of bounds behaviour

                                    return(baselineData[electrode]);
                                }

                                bool MPR121_MOD_t::isNewTouch(unsigned char electrode){
                                    if(electrode>12 || !isInited()) return(false); // avoid out of bounds behaviour
                                        return((getLastTouchData(electrode) == false) && (getTouchData(electrode) == true));
                                    }

                                    bool MPR121_MOD_t::isNewRelease(unsigned char electrode){
                                        if(electrode>12 || !isInited()) return(false); // avoid out of bounds behaviour
                                            return((getLastTouchData(electrode) == true) && (getTouchData(electrode) == false));
                                        }

                                        void MPR121_MOD_t::updateAll(){
                                            updateTouchData();
                                            updateBaselineData();
                                            updateFilteredData();
                                        }

                                        void MPR121_MOD_t::setTouchThreshold(unsigned char val){
                                            if(!isInited()) return;
                                            bool wasRunning = running;
                                            ...
                                            void MPR121_MOD_t::setTouchThreshold(unsigned char electrode, unsigned char val){
                                                ...
                                                void MPR121_MOD_t::setReleaseThreshold(unsigned char val){
                                                    if(!isInited()) return;
                                                    bool wasRunning = running;
                                                    ...
                                                    void MPR121_MOD_t::setReleaseThreshold(unsigned char electrode, unsigned char val){
                                                        if(electrode>12 || !isInited()) return; // avoid out of bounds behaviour
                                                        ...
                                                        unsigned char MPR121_MOD_t::getTouchThreshold(unsigned char electrode){
                                                            if(electrode>12 || !isInited()) return(0xFF); // avoid out of bounds behaviour
                                                                return(getRegister(E0TTH+(electrode<<1)));
                                                            }
                                                            unsigned char MPR121_MOD_t::getReleaseThreshold(unsigned char electrode){
                                                                if(electrode>12 || !isInited()) return(0xFF); // avoid out of bounds behaviour
                                                                    return(getRegister(E0RTH+(electrode<<1)));
                                                                }

                                                                void MPR121_MOD_t::setInterruptPin(unsigned char pin){
                                                                ...
                                                                void MPR121_MOD_t::setNumDigPins(unsigned char numPins){
                                                                    if(!isInited()) return;
                                                                    bool wasRunning = running;
                                                                    ...
                                                                    void MPR121_MOD_t::pinMode(unsigned char electrode, mpr121_pinf_t mode){
                                                                    ...
                                                                    void MPR121_MOD_t::pinMode(unsigned char electrode, int mode){
                                                                        if(!isInited()) return;
                                                                    ...
                                                                    void MPR121_MOD_t::digitalWrite(unsigned char electrode, unsigned char val){
                                                                    ...
                                                                    void MPR121_MOD_t::digitalToggle(unsigned char electrode){
                                                                    ...
                                                                    bool MPR121_MOD_t::digitalRead(unsigned char electrode){
...
void MPR121_MOD_t::analogWrite(unsigned char electrode, unsigned char value){
